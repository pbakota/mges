...
final class DrawTextOptions {
  ...
  DrawTextOptions({bool? shadow, String? shadowColor, num? scale})
    : this.shadow = shadow ?? false, this.shadowColor = shadowColor ?? '#00000000', this.scale = scale ?? GLYPH_SCALE;
}

class BitmapText {
  Map<String, ImageSlice> _glyphs = new Map<String, ImageSlice>();
  late CanvasElement _buffer;
  late CanvasRenderingContext2D _bufferCtx;

  BitmapText(ImageElement image) {
    this._buffer = document.createElement('canvas') as CanvasElement;
    this._buffer.width = 640+2;
    this._buffer.height = (GLYPH_SCALE * 8 + 2 * GLYPH_SCALE).toInt();
    this._bufferCtx = this._buffer.context2D;
  ...
  {
    final opts = options ?? new DrawTextOptions();

    this._bufferCtx.save();
    this._bufferCtx.clearRect(x, y, this._buffer.width!, this._buffer.height!);

    var xx = 0; var yy = 0;
    for(var c = 0; c<text.length;++c) {
      this._glyphs[text[c]]?.draw(this._bufferCtx, xx, yy, options: new ImageSliceDrawOptions(scale: opts.scale));
